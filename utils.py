...
custom_transform = transforms.Compose([
    ...
    # transforms.RandomAffine(degrees=0, translate=(0.25, 0.25), scale=(0.6, 1.8), shear=30),
    # transforms.ColorJitter(brightness=1.0, contrast=0.4, saturation=0.7, hue=0.015),
...
def feature_extraction_global_regression(arg, train_data, test_data, model, device):
    ...
    dataloader_Images_train = DataLoader(Images_train, batch_size=batch_size, shuffle=False, num_workers=0)

    Images_test = ImageLoader(arg, test_data)
    dataloader_Images_test = DataLoader(Images_test, batch_size=batch_size, shuffle=False, num_workers=0)
    ...
    with torch.no_grad():
        ...
        for i, data in enumerate(tqdm(dataloader_Images_train)):
            ...
            outputs = (model('extraction', x=inputs))

            # outputs_numpy = outputs.cpu().detach().numpy().reshape(-1, 512)
            train_torch.append(outputs)
        features['train'] = torch.cat(train_torch)
        ...
        for i, data in enumerate(tqdm(dataloader_Images_test)):
            ...
            # outputs_numpy = outputs.cpu().detach().numpy().reshape(-1, 512)
            test_torch.append(outputs)
        features['test'] = torch.cat(test_torch)
    features_train = features["train"].cpu().detach().numpy()
    features_test = features["test"].cpu().detach().numpy()
   
    # Lưu features vào file hoặc thực hiện các thao tác khác
...
def get_gt(age_y1, age_y2, age_x):
    ...
    return gt
def get_pairs (arg,train_data,features_train,features_test,pred,model,device,load):
    data_age_max = int(train_data['age'].max())
    data_age = train_data['age'].to_numpy().astype('int')
    data_test_age = np.array(pred).astype('int')
    lb_list, up_list = get_age_bounds(int(data_age_max), np.unique(data_age), arg.tau)
    feat_train = torch.as_tensor(features_train, dtype=torch.float32).to(device).squeeze()
    feat_test = torch.as_tensor(features_test, dtype=torch.float32).to(device).squeeze()
    loss_total = []
    index_y2_total = []
    for i, age_y1 in enumerate(tqdm(data_age)):
        ...
        if load == False:
            ...
            feat_y1 = feat_train[i].repeat(len(index_y2), 1).reshape(-1, 960, 1, 1)
            feat_y2 = feat_train[index_y2].reshape(-1, 960, 1, 1)

            for index in index_test:
                feat_x = feat_test.repeat(1, len(index_y2)).reshape(-1, 960, 1, 1)

                for k in range(0, len(feat_y1), batch_size):
                    batch = min(batch_size, len(feat_y1))
                    outputs = model('test', x_1_1=feat_y1[k:k + batch], x_1_2=feat_y2[k:k + batch],
                                    x_2=feat_x[k:k + batch])
                    loss_tmp.extend(outputs.squeeze().cpu().detach().numpy().reshape(-1))
            outputs = np.array(loss_tmp)
            loss = np.power((outputs - gt_total), 2).reshape(len(index_test), len(index_y2)).mean()
            loss_total.append(loss)
    return loss_total
def get_best_pairs_global_regression(arg, train_data, test_data, features_train,features_test, model, device, load):
...
######################### MWR ########################
def initial_prediction(train_data, test_data, features_train, features_test, n_neighbors):
    f_tr, f_te = (features_train, features_test)
    if len(f_te) == 1:
...
def MWR_global_regression(arg, train_data, test_data, features_train,features_test, refer_idx, refer_idx_pair, initial_prediction, model, device):
    ...
    f_tr = torch.as_tensor(features_train).to(device)
    f_te = torch.as_tensor(features_test).to(device)
    ...
    with torch.no_grad():
        for i in tqdm(range(len(test_data)), "Test"):
            ...
            while True:
                ...
                feature_1, feature_2, feature_test = f_tr[idx_1_final].reshape(-1, 960, 1, 1), f_tr[idx_2_final].reshape(-1, 960, 1, 1), \
                                                     f_te[test_duplicate].reshape(-1, 960, 1, 1)
...
def get_results(arg, test_data, pred_age):
    ...
    print('Dataset, Setting: %s, MAE: %.2f, CS: %.3f'%( Error.mean(), CS.mean()))
