...
def create_model(arg, model_name):
    ...
    if model_name == 'Global_Regressor':
        ...
        model = Global_Regressor().to(device)

    if model_name == 'Local_Regressor':
        print('Get Local_Regressor')
        model = Local_Regressor(arg).to(device)
...
class Local_Regressor(nn.Module):
    def __init__(self, arg):
        ...
        self.encoder = nn.ModuleList([ptcv_get_model("mobilenetv3_large_w1", pretrained=True) for _ in range(self.reg_num)])
        self.avg_pool = nn.AvgPool2d(kernel_size=7)
        self.regressor = nn.ModuleList([Regressor(2880, 512) for _ in range(self.reg_num)])
